...
function configurationPage(
  ...
  return [
    ...
    '}',
    'function updateSummaryIcon(card){',
    'var main=card.querySelector(".summary-main");',
    'if(!main){return;}',
    'var oldIcon=main.querySelector(".summary-icon");',
    'if(oldIcon){main.removeChild(oldIcon);}',
    'var symbol=numberValue(card,"symbol");',
    'var shape=numberValue(card,"shape");',
    'var color=numberValue(card,"color");',
    'var color2=numberValue(card,"color2");',
    ...
    'var color2Valid=color2>=192&&color2<=255;',
    'var icon=null;',
    'if(symbol===0&&shape>=0&&shape<=4&&colorValid){',
    'icon=document.createElement("span");',
    'icon.className="summary-icon summary-shape-"+shape;',
    'if(shape===4&&color2Valid){icon.style.backgroundImage="linear-gradient(90deg,"+pebbleColorHex(color)+" 0 50%,"+pebbleColorHex(color2)+" 50% 100%)";}',
    'else{icon.style.backgroundColor=pebbleColorHex(color);}',
    '}else if(symbol===1&&colorValid&&color2Valid){',
    'icon=document.createElement("span");',
    'icon.className="summary-icon summary-pen";',
    'icon.style.backgroundImage=summaryPenBackground(color,color2);',
    '}',
    ...
    'var sub=medicationSummaryText(time,schedule,day,intervalHours,intervalStart,dosage,effect,quantity,enabled);',
    ...
    '}else if(med.symbol===1&&med.color>=192&&med.color<=255&&med.color2>=192&&med.color2<=255){summaryIcon="<span class=\\"summary-icon summary-pen\\" style=\\"background-image:"+summaryPenBackground(med.color,med.color2)+"\\"></span>";}',
    'var sub=medicationSummaryText(med.time,med.schedule,med.day,med.intervalHours,med.intervalStart,dosage,effect,med.quantity,med.enabled);',
    ...
    'card.querySelector("[data-field=\\"schedule\\"]").onchange=(function(item){return function(){updateDayFields(item);updateMedicationSummary(item);};})(card);',
    'card.querySelector("[data-field=intervalHours]").onchange=(function(item){return function(){updateMedicationSummary(item);};})(card);',
    'card.querySelector("[data-field=intervalStart]").oninput=(function(item){return function(){updateMedicationSummary(item);};})(card);',
    'card.querySelector("[data-field=intervalStart]").onchange=(function(item){return function(){updateMedicationSummary(item);};})(card);',
    ...
    'card.querySelector("[data-field=\\"enabled\\"]").onchange=(function(item){return function(){updateMedicationSummary(item);};})(card);',
    ...
    '};',
    'function daypartMinutesOrDefault(id,fallback){var value=window.__timeToMinutes(document.getElementById(id).value);return value<0?fallback:value;}',
    'document.getElementById("settings").onsubmit=function(event){',
    'event.preventDefault();',
    'var values={',
    'morning:daypartMinutesOrDefault("morning",360),',
    'noon:daypartMinutesOrDefault("noon",720),',
    'evening:daypartMinutesOrDefault("evening",1080),',
    'night:daypartMinutesOrDefault("night",1320)',
    '};',
    ...
    'function bindDaypartDefault(id,fallback){var input=document.getElementById(id);function refresh(){var value=window.__timeToMinutes(input.value);if(value<0){input.value=window.__minutesToTime(fallback);value=fallback;}input.style.color=value===fallback?"#888":"";}input.oninput=refresh;input.onchange=refresh;refresh();}',
    'bindDaypartDefault("morning",360);',
    'bindDaypartDefault("noon",720);',
    'bindDaypartDefault("evening",1080);',
    'bindDaypartDefault("night",1320);',
    'render(-1);',
    '</script>',
    '</body>',
    '</html>'
